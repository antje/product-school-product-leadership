...
> I chose to sequence **memory-grounded check-ins (item 6), crisis mode (item 9), and instant availability (items 14 and 4)** first because they are, in order, the differentiator, the license to operate, and the promise made physical. Cost-of-delay reasoning drove all three. Every quarter without memory-grounded conversations is another quarter where our "felt understood" claim looks no different from ChatGPT's. Shipping any conversational feature before the safety gate exists is a risk we cannot insure against. And the dormant-graduate group shrinks over time, so anything that makes returning harder, a 4.2-second load or a notification at the wrong hour, costs us people we can never get back. The unconventional call is treating performance as a Rock rather than sand: for a product whose winning aspiration is "the first thing you talk to when something feels off," a slow open is not a polish issue, it is the first thing the user experiences.
>
> I pushed out **pattern insights, the therapist handoff, the pricing pilot, the onboarding redesign, and life-transition arcs** because each has a hard dependency on a Now bet proving out first. WSJF (scored in the second appendix) drove the sequencing. Pattern insights are valuable, but the job is bigger than it looks until the memory engine has real history to work with. The therapist handoff, which is the strategic half of the CEO's premium-tier request, needs the crisis-mode escalation path built first. The pricing pilot would tell us a lot, but running it before KR2 shows the companion works would just measure noise. And redesigning onboarding now would polish the product we are moving away from.
>
> I cut **daily streaks (item 2)** entirely because they violate the strategy's one hard no: the money from engagement mechanics is borrowed against user trust, and we are not willing to spend that trust. I cut **content library expansion (item 12)** because it deepens the product users graduate out of; investment in yesterday's value prop has negative strategic return whatever its matrix score says. I cut **the social sharing layer (item 3)** because mental-health progress is not social currency and sharing pressure works against the privacy promise the memory engine depends on. And I cut **Fable for Teams (item 13)** for this cycle, the genuinely uncomfortable call given the revenue attached, because employer reporting incentives point directly at both our hard no and our privacy promise. It is the first candidate for reconsideration once KR2 proves the companion works; the other three cuts are permanent.
...
| 14 | Load time 4.2s to under 2s on 4G | Engineering | 4 | 2 | Do Now | 🪨 Rock #3 → KR1. Opening fast is how trust gets demonstrated rather than claimed. |
