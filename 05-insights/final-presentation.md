# Final Presentation: Fable, the companion bet

> Module 6 · Final submission. A title card plus five slides, one per deliverable. Content is the source of truth here; the deck renders it.
>
...
**How a mental-wellness app keeps growing when its users get better**

Product Leadership Certification · Final Project
Product School · August 2026 cohort

Antje Barth

> Say: open by naming the paradox. Fable's users leave because it worked. Everything that follows is the answer to that.

---

## Slide 1 · Product Strategy

### Fable's real competitor is the user deciding they are done

**The diagnosis.** Fable works. Adults resolve the anxiety episode they came for in 90 to 120 days, conclude they are finished, and stop opening the app. 4.2M users, growth flat. You cannot fix that with engagement mechanics, because the mechanics would be arguing with a user who is right.

**The bet.** Fable becomes a clinically grounded AI companion whose value compounds through longitudinal personal memory. It wins the everyday "something feels off" moment for people it already has history with.

**How we win.** ChatGPT starts every mental-health conversation cold. Calm and Headspace hold attention, not history. After a year with Fable, switching means starting over with a stranger.
...
**The seam this charter had to close.** Retention's fastest levers and the hard no look identical inside a real ticket. Both are a notification. So the line is written: a check-in is earned return if a signal specific to that user triggers it, and manufactured dependency if elapsed time or a frequency target triggers it.

> Say: this is the module where I found out my strategy's hard no was unusable as written.
...
## Slide 5 · Individual Insights

### A strategy is only as real as the number that would end it

**Friction.** The hard no took two minutes to write and four modules to make usable. Module 2 made me cut real revenue. Module 3 showed the rule could not be applied to a ticket. Module 4 caught me about to concede the loophole myself, because time since last open is a user-specific signal.
